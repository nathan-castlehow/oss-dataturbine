 ...
 */
public class ExpressionDataPanel extends AbstractDataPanel
implements ActionListener, ListSelectionListener {

	static Log log = LogFactory.getLog(ExpressionDataPanel.class.getName());
	
	private double front = -1.0; // the time in the sweep of data
	...
	JComponent initPanel() {
		...
		adjustButtons(true,false,false); // check, reset, details
		...
		// holder for the list of channels
...
//		panel.setLayout(g);
	...
	private void adjustButtons
		(boolean check, boolean reset, boolean details)
	{
	...
	public void postTime(double time)
	{
		...
		if (channelMap == null) {
			...
			// log.info("No data yet " + DataViewer.formatDate(time) + ".");		
			return;
		}
		...
		while (i.hasNext()) {
			...
			if (channelIndex != -1) {
				// log.info("  Data on channel = " + channelName + ".");
				// get channel cover for data
	...
	private void process() // do as much as we can
	{
		if (channelList == null)
		{
			// bad mojo!
		...
		while (ok)
		{
			// advance to front
			for (int i = 0; i < cc.length; i++)
				ok = ok && cc[i].advance(front);
			if (!ok) break;
			
			// get data from each channel and update front
			for (int i = 0; i < cc.length; i++)
			{
		...
		if (outIndex >= outTime.length)
		{
			...
			if (connected())
			{
				...
				catch (SAPIException e)
				{
					...
					setMessage("Failed to send results. Exception = " +
						e.getMessage() + ". See Details." );
					notReady();
				}
			}
			else
	...
	{
		// log.info("Called postData " + DataViewer.formatDate(time) + ".");
		super.postData(channelMap);
	}

	public void postState(int s1, int s2)
	{
		// log.info("Called postState " + s1 + " --> " + s2 + ".");		
		super.postState(s1,s2);	
	}

	void clearData()
	{
		// log.info("Called ClearData.");
	}
	...
	public void actionPerformed(ActionEvent ev) {
		...
		if (arg.equals(CHECK))
		{
			if (parseExpression() && channelsOk()) ready();
			else notReady();
		}
	...
	private void notReady()
	{
		...
		adjustButtons(true,false,details); // check, reset, details
		parseTree = null; // force a restart
		disconnect();
		running = false;
	}
	...
	private void ready()
	{
		...
		adjustButtons(false,true,false); // check, reset, details
		running = true;
		setMessage("Expression is ready to evaluate; " +
			"start data to see output on " + outSourceName + 
			"/" + outChannelName + ".");
		connect();
	}

	private boolean setup() {
		if ((parseTree == null) && (!parseExpression()))
			return false;
	...
	private boolean parseExpression() {
		...
		{
			parseTree =
				new Expression(new java.io.BufferedReader(new java.io.StringReader(exStr)));
			parseTree.setup();
		...
		{
			parseTree = null;
			setMessage("The parse of expresison failed (" +
				exStr + ") -- click on 'Details' for details.\n");
			setDetailFromException(e);
			return false;
		}
	...
	private boolean channelsOk() {
		...
		if (parseTree == null) 
		{
			setMessage("Expression not set up in parse tree. Use '" +
				CHECK + "'.");
			return false;
		} 
		try {
			boolean[] expressionChannels = parseTree.getChannelList();
			//log.info("expressionChannels.length = " + expressionChannels.length);
			//log.info("channelList.size() = " + channelList.size());
			if (expressionChannels.length > channelList.size())
			{
				if (channelList.size() == 0)
					setMessage("Channels not set. Drag and Drop channels to channel list.");
				else
					setMessage("Missing some channel or channels needed in the expression");
				return false;
			}
		} catch (Throwable e) {
			setDetailFromException(e);
	...
	private boolean connect()
	{
		...
		try
		{
			...
			source.OpenRBNBConnection(
				DataViewer.getRBNBHostName() + ":" + DataViewer.getRBNBPort(),
				outSourceName);
		}
		catch (SAPIException e)
		{
	...
	private class ChannelCover
	{
		...
		public boolean hasData(){ return currentTime != null;}
		
		public double getData(){ return currentData[index];}
		public double getTime(){ return currentTime[index];}
		...
		{ // assume that next time exists
			int probe = index + 1;
			if (probe < currentTime.length) return currentTime[probe];
			if (keep.size() == 0)
				log.error("in nextTime with empty keep and no data!");
