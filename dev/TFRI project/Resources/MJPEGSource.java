...
public abstract class MJPEGSource extends RBNBSource {
  ...
  protected boolean setBaseArgs(CommandLine cmd) {
    ...
    if (cmd.hasOption ('m')) {
      if (! cmd.hasOption ('r')) {
        writeMessage("The -m parameter is only used by this program in " +
		...
		if (cmd.hasOption ('r')) {
		 // if the user specified frames, over-ride this calcultaion
		  if (cmd.hasOption('Z')) {
		     writeMessage("Note: Value specified from -Z will be " +
		           "used to create the ring buffer, rather than the " +
		           "archive value calculated from the -r parameter.");
