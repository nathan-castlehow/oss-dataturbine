...

	...
	if (parameterI >= parametersStart) {
		...
		try {
		    ...
		    setBuildDate
			((com.rbnb.compat.Utilities.SimpleDateFormat
			    ("MMM dd yyyy HH:mm:ss z", date)));
			     //java.util.Locale.US)).parse(date));
		} catch (Exception e) {//java.text.ParseException e) {
...

	...
	if ((getBuildDate() != null) &&
		...
		/*(new java.text.SimpleDateFormat
			...
			com.rbnb.compat.Utilities.dateFormat(getBuildDate());
	    osI.writeUTF(date);
	}
	...
	*/
	...
	public Object clone()
	{
		PeerServer clonedR = new PeerServer();
		cloned(clonedR);
	...
	{
		super.cloned(o);
		PeerServer clonedR = (PeerServer)o;
		clonedR.activeShortcuts = activeShortcuts;
		clonedR.buildDate = buildDate;
		clonedR.buildVersion = buildVersion;
		clonedR.clientSide = clientSide;
		clonedR.passiveShortcuts = passiveShortcuts;
		clonedR.shortcuts = shortcuts;
		clonedR.updateCounter = updateCounter;
	}
}
