...
private boolean alreadyReset=false;
     ...
     * @version 03/14/2003
     */
    java.util.Hashtable rbsChanged = new java.util.Hashtable(10000);
     ...
     * @version 03/14/2003
     */
    java.util.Hashtable rbsToSync = new java.util.Hashtable(10000);
...

	 ...
	 *
	 */
	 public final boolean accept(com.rbnb.compat.File dirI,String nameI) {
...

	...
	com.rbnb.compat.File rbDirectory = new com.rbnb.compat.File(getArchiveDirectory(),
						    "RB1");
	rbDirectory.mkdir();
	...
	for (int idx = 0; idx < filesI.length; ++idx) {
	    filesI[idx].renameTo(new com.rbnb.compat.File(rbDirectory.getPath(),
						  filesI[idx].getName()));
	}
...

	...
	try {
		...
		try {
			...
			while (!reconnecting &&
			       !getTerminateRequested() &&
			       !com.rbnb.compat.Utilities.interrupted(getThread())) {
			    wait(TimerPeriod.NORMAL_WAIT);
			}
		    }
		} catch (java.lang.InterruptedException e) {
...

	...
	try {
		...
		for (int idx = 1; idx <= nRB; ++idx) {
		    com.rbnb.compat.File rbDirectory = new com.rbnb.compat.File
			(getArchiveDirectory() +
			 Archive.SEPARATOR +
			 "RB" + idx);
...

	...
	RingBuffer lastRB = null,
	    ...
	    invalidRB = null,
	    rb;
	com.rbnb.compat.File unrecoverableDirectory = new com.rbnb.compat.File
	    (getArchiveDirectory() + Archive.SEPARATOR + "UNRECOVERABLE"),
	    notrecoveredDirectory = new com.rbnb.compat.File
...
rb.setupTrimTimes(cacheflush,cachetrim,archiveflush,archivetrim);
		...
		try {
		    ...
		    com.rbnb.compat.File oldName = new com.rbnb.compat.File
			(getArchiveDirectory() +
			 Archive.SEPARATOR +
			 "RB" + rb.getIndex()),
			newName = new com.rbnb.compat.File
...

	...
	if (awaiting.size() > 0) {
	    ...
	    java.util.Vector anv = (java.util.Vector)com.rbnb.compat.Utilities.cloneVector( awaiting);
...

	...
	try {
		...
		rbsChanged;
		rbsChanged = new java.util.Hashtable(10000);
	    //rbsChanged.clear();
...
*/
	...
	protected void cloned(Object o)
	{
		super.cloned(o);
		RBO clonedR = (RBO)o;
		clonedR.alreadyReset=alreadyReset;
		clonedR.acceptingAFrame = acceptingAFrame;
		clonedR.accessMode = accessMode;
		clonedR.addingAFrame = addingAFrame;
		clonedR.archiveFrames = archiveFrames;
		clonedR.archiveflush = archiveflush;
		clonedR.archivetrim = archivetrim;
		clonedR.aFileSets = aFileSets;
		clonedR.aFrSFileSet = aFrSFileSet;
		clonedR.archiveKeep = archiveKeep;
		clonedR.archiveSize = archiveSize;
		clonedR.awaiting = awaiting;
		clonedR.cacheFrames = cacheFrames;
		clonedR.cacheflush = cacheflush;
		clonedR.cachetrim = cachetrim;
		clonedR.cFrameSets = cFrameSets;
		clonedR.cFrFrameSet = cFrFrameSet;
		clonedR.cacheKeep = cacheKeep;
		clonedR.cacheSize = cacheSize;
		clonedR.channelToRB = channelToRB;
		clonedR.frame = frame;
		clonedR.frameSets = frameSets;
		clonedR.hasBeenShutdown = hasBeenShutdown;
		clonedR.lastRegistration = lastRegistration;
		clonedR.nFindex = nFindex;
		clonedR.nRBindex = nRBindex;
		clonedR.outOfDate = outOfDate;
		clonedR.performClearCache = performClearCache;
		clonedR.performReset = performReset;
		clonedR.rbsChanged = rbsChanged;
		clonedR.rbsToSync = rbsToSync;
		clonedR.reconnecting = reconnecting;
		clonedR.registered = registered;
		clonedR.registeredExplicitly = registeredExplicitly;
		clonedR.registrationDoor = registrationDoor;
	}
