...

	...
	try {
	    ...
	    for (long idx = getOldest(),
		 ...
		 idx <= endIdx;
		 ++idx) {
		com.rbnb.compat.File fileSetDirectory = new com.rbnb.compat.File
		    (getArchiveDirectory() +
		     Archive.SEPARATOR +
		     "FS" + idx);
...
// __e.printStackTrace();
	...
	int lmeps = 0,
	    ceps = 0;
	com.rbnb.compat.File lastFile = null,
		     validFile = null,
		     invalidFile = null,
		     unrecoverableDirectory = new com.rbnb.compat.File
			 (getArchiveDirectory() +
			  Archive.SEPARATOR +
			  "UNRECOVERABLE"),
	...
	for (int idx = 0,
		...
		if (lastFile == null) {
			...
			try {
			    ...
			    com.rbnb.compat.File newName = new com.rbnb.compat.File
	...
	for (int idx = 0; idx < files.length; ++idx) {
		...
		if (name.equalsIgnoreCase("summary.rbn")) {
		    ...
		    for (lo = 0,
			 ...
			 (lo <= hi);
			 idx1 = (lo + hi)/2) {
			com.rbnb.compat.File other =
			    (com.rbnb.compat.File) fileSets.elementAt(idx1);
