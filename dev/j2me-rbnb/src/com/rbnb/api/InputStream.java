...

	...
	if (sizeI == 0) {
	    ...
	    // old is = new java.io.BufferedInputStream(iStreamI,sizeI);
		is = new java.io.DataInputStream(iStreamI);
	}
	setBinary(binaryI);
    }

...

	...
	if (false) { // mark is actually not supported in j2me is.markSupported()) {
	    is.mark(readlimitI);
...

	...
	throws java.io.IOException
    {
	if ((is != null) && false) { // is.markSupported()) {
	    is.reset();
	} else {
	    markIndex = 0;
	}
...

	...
	int elements, indexR = -1;
	elements = Utilities.getArrayLength(arrayI);
...

	...
	if (getBinary()) {
		...
		if (!Utilities.isWhitespace(cValue)) {
		    baos.write(value);
		    break;
