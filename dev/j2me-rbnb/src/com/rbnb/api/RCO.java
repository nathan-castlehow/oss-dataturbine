...
//	}
	...
	try {
	    ...
	    // getThread().setName("RCO." + myClient.getName());  there is no setname function
		    ...
		    */
			...
			throw new com.rbnb.api.SerializeException
			    ("Received out-of-order ping. Expected " +
			     pingValue + ", got " +
			     (ping.getHasData() ?
			      (new Short(ping.getData()).toString()) :
			      "no counter") +
			     ".");
		    }
		    ++pingValue;
...

	...
	} catch (java.io.IOException e) { //java.net.SocketException e) {
	    try {
