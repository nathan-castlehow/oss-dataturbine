...

	...
	if (message.indexOf("\n") == com.rbnb.compat.Utilities.lastIndexOf(message, "\n")) {
	    //pw.close();
...

	...
	if ((levelI <= getLevel()) &&
	    ...
	    if (com.rbnb.compat.Utilities.lastIndexOf(message, LINESEPARATOR) !=
...

	...
	try {
		...
		if (!(done = (oldest == -1))) {
			...
			new com.rbnb.compat.StringTokenizer
			    (message,
			     LINESEPARATOR);
		    String token;
...

	...
	{
		Log clonedR = (Log)o;
		clonedR.classes = classes;
		clonedR.isRunnable = isRunnable;
		clonedR.level = level;
		clonedR.logDisplay = logDisplay;
	}
}
