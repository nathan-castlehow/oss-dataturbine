...

	...
	if (peers.size() > 0) {
		...
		} catch (java.lang.Exception e) {//java.lang.NoSuchMethodError e) {
		    peerCost = ((peerCostS == null) ?
				1. :
	...
	for (int idx = 0; idx < argsI.length; ) {
		...
		for (idx2 = 2;
		     ((idx2 < argsI[idx].length()) &&
		      com.rbnb.compat.Utilities.isWhitespace(argsI[idx].charAt(idx2)));
		     ++idx2) {}
		...
		if ((idx1 < argsI.length) &&
		    ...
		    com.rbnb.compat.StringTokenizer sTok =
			new com.rbnb.compat.StringTokenizer(arguments,",+");
...

	...
	javax.microedition.io.Connection urlCon = javax.microedition.io.Connector.open(fileName);
	//java.net.URLConnection urlCon = url.openConnection();

	if (urlCon instanceof javax.microedition.io.HttpsConnection) { //url.getProtocol().equalsIgnoreCase("HTTPS")) {
	...
	java.io.InputStreamReader isr = new java.io.InputStreamReader
	    (((javax.microedition.io.HttpConnection)urlCon).openInputStream());
	com.rbnb.compat.LineNumberReader lnr = new com.rbnb.compat.LineNumberReader(isr);
	...
	try {
		...
		com.rbnb.compat.StringTokenizer st = new com.rbnb.compat.StringTokenizer
		    (line,
		     " ");
...

	...
	{
		super.cloned(o);
		Server clonedR = (Server)o;
		clonedR.addressHandler = addressHandler;
		clonedR.serverSide = serverSide;
		clonedR.address = address;
	}
