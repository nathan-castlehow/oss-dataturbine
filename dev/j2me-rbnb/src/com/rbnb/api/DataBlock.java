...

	...
	DataBlock clonedR = new DataBlock(); //super.clone();
	//DataBlock clonedR = super.clone(); 
		clonedR.numPoints = numPoints;
		clonedR.pointSize = pointSize;
	...
	if (clonedR != null) {
		...
		for (int idx = 0; idx < nEntries; ++idx) {
		    ...
		    if (entry instanceof byte[]) {
				...
				byte[] temp = new byte[((byte[])entry).length];
				for(int i = 0; i<((byte[])entry).length; i++)
					temp[i] = ((byte[])entry)[i];
				nEntry = temp;
			//nEntry = ((byte[]) entry).clone();

		    } else if (entry instanceof byte[][]) {
				byte[][] temp = new byte[((byte[][])entry).length][((byte[][])entry)[0].length];
				for(int i = 0; i<((byte[][])entry).length; i++)
				{
					System.arraycopy(((byte[][])entry)[i], 0, temp, 0, ((byte[][])entry)[i].length);
				}
				
			//nEntry = ((byte[][]) entry).clone();

		    } else if (entry instanceof boolean[]) {
				boolean[] temp = new boolean[((boolean[])entry).length];
				System.arraycopy(((boolean[])entry), 0, temp, 0, ((boolean[])entry).length);

			//nEntry = ((boolean[]) entry).clone();

		    } else if (entry instanceof double[]) {
				double[] temp = new double[((double[])entry).length];
				System.arraycopy(((double[])entry), 0, temp, 0, ((double[])entry).length);
			//nEntry = ((double[]) entry).clone();

		    } else if (entry instanceof float[]) {
				float[] temp = new float[((float[])entry).length];
				System.arraycopy(((float[])entry), 0, temp, 0, ((float[])entry).length);
			//nEntry = ((float[]) entry).clone();

		    } else if (entry instanceof int[]) {
				int[] temp = new int[((int[])entry).length];
				System.arraycopy(((int[])entry), 0, temp, 0, ((int[])entry).length);
			//nEntry = ((int[]) entry).clone();

		    } else if (entry instanceof long[]) {
				long[] temp = new long[((long[])entry).length];
				System.arraycopy(((long[])entry), 0, temp, 0, ((long[])entry).length);
			//nEntry = ((long[]) entry).clone();

		    } else if (entry instanceof String) {
				nEntry = entry;
		    } else if (entry instanceof String[]) {
				String[] temp = new String[((String[])entry).length];
				System.arraycopy(((String[])entry), 0, temp, 0, ((String[])entry).length);
...

	...
	if (element instanceof byte[][]) {
	    ...
	    int nElements = Utilities.getArrayLength(element); //java.lang.reflect.Array.getLength(element);
	    if ((element instanceof byte[]) ||
