...
public class DswProcess implements SinkCallBackListener {
  ...
  public DswProcess(String sinkConfigFile, String sinkName, 
      String srcConfigFile, String[] sourceNames){

    helper = new DswEnd2EndHelper(sinkConfigFile, sinkName, 
        srcConfigFile, sourceNames, this);
  ...
  public void callBack(DataPacket dataPkt) {
    ...
    for(int i=0; i<dataPkt.getSize(); i++) {
      ...
      for (int j = 0; j < data.length; j++) {
        ...
        String time = DateFormat.getDateTimeInstance().format(
          new Date((long)(dataPkt.getTimestampAt(i,j))));
        System.out.println("Received|" + helper.sink.getName()  + "|" + chanName +
            "|" + time + "|" + "|" + j + "|" + data[j]);
      }

      // insert them into respective channels 
      ...
      for (int j = 0; j < data.length; j++) {
        System.out.println("Sent|" + sourceNames[0] + "/" +
            convertedChanName + "|" +
    ...
    String time = DateFormat.getDateTimeInstance().format(new Date());
    System.out.println("Flushed|" + sourceNames[0] + "|" +
  ...
  public static void main(String args[]){
    ...
    if(args.length < 4){
      System.err.println("Usage:  " +
          "<sinkConfig xml file>  <sink name> <srcConfig xml> <sourceNames ...>");
      return;
    }
    ...
    DswProcess process = new DswProcess(sinkConfigFile, sinkName, 
        srcConfigFile, sourceNames);
