...
public class SensorMetaData{
  ...
  public void setProperty(String key, String value){
    ...
    if(Constants.SITE_META_DATA_SERVICE_BUOY_ID.equals(key)){
      ...
      throw new IllegalArgumentException("The key \"" + key + "\" is not "+
          "recognized as a parameter for the sensor in the meta data " +
          "information");
    }
  }
  ...
  public void resetInfo(boolean runningInstanceToo){
    ...
    if(runningInstanceToo) {
      // TODO:
      // If the source is already connected, then disconnect
      if(source != null) {
        source.disconnect();
        srcConfig = null;
      }
  ...
  private void parseAndSetDataIds(String ids){
    ...
    StringTokenizer st = new StringTokenizer(
        ids,Constants.SITE_META_DATA_SERVICE_DATA_IDS_DELIM);
    String dataId;
    ...
    while(st.hasMoreTokens()) {
      ...
      // TODO: We assume all elements as double
      // Need to access meta data information for datatype too
      dataIdDatatypeVec.addElement(Constants.DATATYPE_DOUBLE_OBJ);
    }
  }
  ...
  public void printMetaData(int level){
    ...
    for(int i=0;i<dataIdVec.size(); i++) {
      ...
      Debugger.debug(level, "\t " + dataIdVec.elementAt(i) + ":" + 
          dataIdDatatypeVec.elementAt(i));
    }
  }
