...
public class LakeSource {
  ...
  public LakeSource(String configFile) {
    ...
    try {
      ...
      if(Constants.SITE_LISTENER_HEADER_LENGTH_SIZE >4 ) {
        throw new IllegalStateException("The header length size cannot be " +
            "greater than 4 (the number of bytes in integer in Java)");
      }
      if(configFile != null) {
  ...
  public byte[] readFromSock(Socket sock){
    ...
    try {
      ...
      int len = Utility.readIntoInt(buffer, 0, 
            Constants.SITE_LISTENER_HEADER_LENGTH_SIZE);
  ...
  private void initializeSource(SensorMetaData smd, boolean overwriteRunningInstance){
    ...
    if(connected == false){
      throw new IllegalStateException("Could not connect the source "+
          "\"" + smd.getId() + "\"");
    }
  ...
  public void broadcastOnControlChannel(SensorMetaData smd){
    ...
    TurbineSrcConfig controlSrcConfig = new TurbineSrcConfig(
        Constants.LAKE_CONTROL_SOURCE_NAME);
    ...
    TurbineServer controlServer = new TurbineServer(currServer.getServerAddr(),
        currServer.getUsername(), currServer.getPassword());
    ...
    if("127.0.0.1".equals(serverAddr) || "localhost".equals(serverAddr)) {
      ...
      System.err.println("WARNING: WARNING: WARNING: Server address is local " +
          "address. IP address being sent (" + serverAddr +") to sink may be " +
          "a loopback address. " +
          "In such a case, sink WILL not work if it is not on the same machine " +
          "as the server");
    }
    ...
    String message = Constants.LAKE_CONTROL_LOOKUP_PREFIX  + 
      smd.getId() + Constants.LAKE_CONTROL_SEPARATOR  + serverAddr + 
      portStr + Constants.LAKE_CONTROL_SEPARATOR + 
      username + Constants.LAKE_CONTROL_SEPARATOR +
      password + Constants.LAKE_CONTROL_SEPARATOR +
      smd.getWebServiceString();
    src.insertData(chnlIndex.elementAt(0).intValue(),
        (Object)message);
    src.flush();
    src.disconnect();
  }
  ...
  private void configureSrcConfig(SensorMetaData smd, SrcConfig srcConfig){
    ...
    if(srcConfig instanceof TurbineSrcConfig) {
      ((TurbineSrcConfig)srcConfig).resetChannelVecs(smd.getChannels(), 
          smd.getChannelDatatypes());
    }
    else {
      throw new IllegalArgumentException( "Currently only data turbine is " +
          "supported");
    }
    ...
    try {
      ...
      switch(type) {
        ...
        case Constants.SITE_LISTENER_GET_META_DATA_TYPE:
        {
          ...
          sendSmd(sock,smd);
          // If there is any change in the meta data
          // then re-connect the source
        ...
        default:
          throw new IllegalStateException("Illegal Type of packet: Only " +
              "SEND_DATA and GET_META_DATA are supported currently.");
  ...
  public synchronized void ackAndCheckForFeedback(Socket sock, SensorMetaData smd){
    ...
    // Packet size DOES NOT INCLUDE the pktSize that is sent as a short
  ...
  public void send(Socket sock, byte[] buf){
    try {
      ...
      ioe.printStackTrace();
      throw new IllegalStateException("Error sending: Could not send via "+
          "the given socket");
    }
  }
  ...
  public void sendSmd(Socket sock, SensorMetaData smd){
    // We send:
    // 1. Id 2. Number of channels 3. Data Types of channels
    // 4. The whole webservice string. This can be optimized to send just
    // the necessary ones, but data logger agent needs all info.
    String id = smd.getId();
    short len = (short)id.length();

    // the first channel is the timestamp channel. No need to send it
    ...
    byte[] finalBuff = new byte[idLenArr.length + numChannelsArr.length
      + idArr.length + idLenArr.length*datatypes.size() + 
      webSerStrLenArr.length + webSerStrArr.length ];
    int offset = 0;
    ...
    offset += idArr.length;
    System.arraycopy(numChannelsArr, 0, finalBuff, offset, 
        numChannelsArr.length);
    ...
    for(int i=1;i < datatypes.size(); i++){
      ...
      // switch(i){
      //   case 0:
      //     dtArr = Utility.shortToByteArray((short)100);
      //     break;

      //   case 1:
      //     dtArr = Utility.shortToByteArray((short)255);
      //     break;

      //   case 2:
      //     dtArr = Utility.shortToByteArray((short)555);
      //     break;

      //   default:
      //     dtArr = Utility.shortToByteArray((short)-5);
      //     break;
      // }
      byte[] dtArr = Utility.shortToByteArray(
          (short)(((Integer)datatypes.elementAt(i)).intValue()));
      
      System.arraycopy(dtArr, 0, finalBuff, offset, 
          dtArr.length);
      offset += dtArr.length;
    }
  ...
  private SensorMetaData handleSendData(StructureInputStream sis) {
    ...
    try {
      ...
      SensorMetaData smd = 
        SensorMetaDataManager.getInstance().getSensorMetaData(key);

      if(smd == null) {

        throw new IllegalArgumentException("There is no sensor registered" +
            "with the id \"" + key + "\"");
      }
  ...
  public boolean handleSensorSpecificData(SensorMetaData smd, 
      StructureInputStream sis) {

    try{

      int dataLen = sis.readUnsignedShort();
      // offset += Constants.SITE_LISTENER_SEND_DATA_DATA_LENGTH_SIZE;
      Debugger.debug(Debugger.TRACE, "Sis:dataLen=" + dataLen);
      // timestamp is sent as a 64 bit number(long long)
      ...
      for(int i=0; i< smd.getNumChannels(); i++) {
        ...
        switch(dt) {
          ...
          default:
            throw new IllegalStateException("Currently, support is " + 
                "provided only to double data type");
        }
      }
      smd.getSource().flush();
      return true;
  ...
  public synchronized void appendFeedbackMsg(String srcName, 
      Date time, String feedbackMsg){
