...
public class SiteSource
{
  ...
  **/
  ...
  public boolean connect() {
    ...
    // TODO: If the feedback sink is already intiated
    // then disconnect that sink first.
    
    // Initialize the feedback sink
  ...
  private void createFeedbackSink() {
    ...
    // Feedback sinkName will be source + feedbackSink
    ...
    // Adds the server (it is still not present)
    TurbineServer server = sinkConfig.getTurbineServer(src.getServerName(), 
        src.getUsername(), src.getPassword());
    // Do not add feedback for the feedbackSink (recursion)

    // Now add the request path
    String reqPath = src.getName() + Constants.FEEDBACK_SRC_SUFFIX + 
          "/" + Constants.FEEDBACK_CHANNEL_NAME;
    ...
    server.addSinkChannel(reqPath, datatype, new Integer(reqMode), 
        intervalOrTimeout);
    TurbineRawSink sink = new TurbineRawSink(server, feedbackSinkName, 
        feedbackListener);
