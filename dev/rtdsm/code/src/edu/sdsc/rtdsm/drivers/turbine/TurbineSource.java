 ...
 * 
 */
public class TurbineSource extends TurbineClient 
  ...
  public void initSrc ( SrcConfig srcConfigVal){
    ...
    if(! (srcConfigVal instanceof TurbineSrcConfig)){
      ...
      throw new IllegalArgumentException("The config parameter has to be " +
          "an instance of TurbineSrcConfig");
    }
    ...
    src = new Source(srcConfig.cacheSize, srcConfig.archiveMode, 
        srcConfig.archiveSize);
    setClient(src);
  ...
  public void insertData (int channel, Object data) {
    ...
    try {
      Debugger.debug( Debugger.RECORD,"Call to insert data:" +
          srcConfig.getName() +":" + channel);
      ...
      switch(((Integer)srcChannelDataTypes.elementAt(channel)).intValue()){
        ...
        case Constants.DATATYPE_DOUBLE:
          ...
          map.PutDataAsFloat64(
              srcChannelIndicies.elementAt(channel).intValue(), dataDouble);
          Debugger.debug( Debugger.RECORD, src.GetClientName()  + ": Inserted data at chnl(" + 
              ((Integer)srcChannelIndicies.elementAt(channel)) + " " +  
              srcChannelNames.elementAt(channel) + ")=");
        ...
        case Constants.DATATYPE_STRING:
          ...
          map.PutDataAsString(
              srcChannelIndicies.elementAt(channel).intValue(), dataString);
          Debugger.debug( Debugger.RECORD, src.GetClientName()  + ": Inserted data at chnl(" + 
              ((Integer)srcChannelIndicies.elementAt(channel)) + " " +  
              srcChannelNames.elementAt(channel) + ")=" + dataString);
          break;

        default:
          throw new UnsupportedOperationException("The datatype specified " +
              "is not supported");
      }
    }
