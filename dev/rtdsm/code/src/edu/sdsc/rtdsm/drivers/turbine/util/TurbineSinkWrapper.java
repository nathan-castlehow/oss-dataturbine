...
public class TurbineSinkWrapper extends TurbineClientWrapper {
  ...
  public void addChannel(String channelName, Integer datatype, Integer mode, 
      Integer intervalOrTimeout){
  ...
  public int getRequestMode(){
    // We support only one request mode per sink now
    // Either poll or monitor or fetch
    if(channelModeVec == null || channelModeVec.size() == 0) {
      throw new IllegalStateException("Trying to fetch the request mode " +
          "for a sink with no channels");
    }
    return channelModeVec.elementAt(0).intValue();
  }

  public int getTimeout(){
    // We support only one request mode/timeout/interval per sink now
    // Either poll or monitor or fetch
  ...
  public void resetChannelVecs(Vector<String> channelVec, 
    ...
    if(channelVec.size() != reqModeVec.size() || channelVec.size() != intervalOrToutVec.size()) {
      throw new IllegalArgumentException( "The channel vector and its " +
          "request modes and timeouts/intervals should be of the same size");
    }
