 ...
 * 
 */
abstract public class TurbineClient {
  // Fields
  // 
  ...
  public TurbineClient (TurbineServer server, String cliName) { 
    ...
    if(null == server) {
      throw new IllegalArgumentException("TurbineClient should be called with a "+
        "valid server argument");
    }
    this.server = server;
  ...
  public boolean connect () {
    ...
    try {
      ...
      cli.OpenRBNBConnection(server.serverAddr,
          cliName, server.userName, 
          server.password);
      return true;
    }
  ...
  public Vector<Integer> addChannels (Vector<String> channelNames) {
    ...
    if (map != null) {
      throw new IllegalStateException("Channels are being added after the " +
          "Channel map has been initialized. Duplicate additions?");
    }
    map = new ChannelMap();
  ...
  public int getChannelIndex(String chName){
    ...
    if (chInt == null ) {
      throw new IllegalArgumentException("Channel by name \"" + chName +
          "\" does not exist in Client\"" + cliName + "\".");
    }
