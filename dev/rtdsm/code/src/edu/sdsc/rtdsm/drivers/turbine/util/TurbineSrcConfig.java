      ...
      if (node_k.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if(tagName.equals(TurbineSrcConfig.STORAGE_TAG)) {
          ...
          if(ele.hasAttribute(TurbineSrcConfig.CACHE_SIZE_TAG)) {
            cacheSize = Integer.parseInt(ele.getAttribute(
                TurbineSrcConfig.CACHE_SIZE_TAG));
          }

          if(ele.hasAttribute(TurbineSrcConfig.ARCHIVE_MODE_TAG)) {
            archiveMode = ele.getAttribute(
                TurbineSrcConfig.ARCHIVE_MODE_TAG);
          }

          if(ele.hasAttribute(TurbineSrcConfig.ARCHIVE_SIZE_TAG)) {

            archiveSize = Integer.parseInt(ele.getAttribute(
                TurbineSrcConfig.ARCHIVE_SIZE_TAG));
          }
    ...
    if(serverNode.getNodeType() == Node.ELEMENT_NODE &&
      ...
      if(! serverEle.hasAttribute(TurbineSrcConfig.URI_TAG)){
        throw new IllegalStateException("The server element in " +
            ...
            "has missing attribute: " + TurbineSrcConfig.URI_TAG);
      }
      server.serverAddr = serverEle.getAttribute(TurbineSrcConfig.URI_TAG);

      if(serverEle.hasAttribute(TurbineSrcConfig.USERNAME_TAG)){

        server.userName = serverEle.getAttribute(
            TurbineSrcConfig.USERNAME_TAG);
      }
      if(serverEle.hasAttribute(TurbineSrcConfig.PASSWORD_TAG)){

        server.password = serverEle.getAttribute(
            TurbineSrcConfig.PASSWORD_TAG);
      }
      ...
      if(channelNode.getNodeType() == Node.ELEMENT_NODE &&
        ...
        if(! channelEle.hasAttribute(TurbineSrcConfig.CH_NAME_TAG)){
          ...
          throw new IllegalStateException("The channel element in " +
              " the data config xml document has "
            + "has missing attribute: " + TurbineSrcConfig.CH_NAME_TAG);
        }

        channelNames.add(channelEle.getAttribute(
              TurbineSrcConfig.CH_NAME_TAG));

        if(! channelEle.hasAttribute(
              TurbineSrcConfig.CH_DATATYPE_TAG)){
