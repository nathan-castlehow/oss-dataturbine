...
public class LakeSink implements SinkCallBackListener{
  ...
  /** The handle to the Sink object that will be used to connect to the ORB */
  // DswSink sink;

  /** The handle to the configuration storage class which stores the
  ...
  Hashtable<String, SensorMetaData> smdHash = 
   ...
   **/
  Hashtable<String,Integer> numPktsPerSrcHash = 
  ...
  public void connect() {
    ...
    if(lsc == null) {
      throw new IllegalStateException("The LakeSinkConfig object " +
          "has not yet been initialized for the Sink");
    }
    LakeSinkControlChannelListener controlListener = 
      lsc.getControlChannelListener();
    if(controlListener == null) {
      throw new IllegalStateException("The control channel has not yet " +
          "been initialized for the Sink");
    }
    connected = true;
    // This method does not reutrn
  ...
  public synchronized void callBack(DataPacket dataPkt) {
    ...
    for(int i=0; i<dataPkt.getSize(); i++) {
      ...
      for (int j = 0; j < data.length; j++) {
        ...
        Debugger.debug(Debugger.TRACE, "Received|" + getSink().getName()  + "|" + chanName +
            "|" + time + "|" + "|" + j + "|" + data[j]);
        // Now update into db
        // TODO: We can batch these updates and insert it once. However,
        // timestamp many change for each double. 
        // updateIntoDb(chanName, (Object)new Double(data[j]), 
        //     timestamp.getTime());
      }
  ...
  public void sendFeedbackIfNecessary() {
    ...
    if(getSink().isFeedbackEnabled()) {
      ...
      while(keys.hasMoreElements()) {
        ...
        Debugger.debug(Debugger.TRACE, "FEEDBACK: enabledForSrc(" + src + 
            ")=" + getSink().isFeedbackEnabledForSource(src));
        if(getSink().isFeedbackEnabledForSource(src)) {
          ...
          if(value >= 10) {
            // The source has sent more than 10 packets
            // Send feedback
   ...
   */
  public Hashtable<String,SourceData> 
    convertDataPktToTimeStampedTree(DataPacket dataPkt) {

    Hashtable<String,SourceData> srcHash = 
      new Hashtable<String,SourceData>();
    //FeedbackTesting
    Hashtable<String, Integer> recentNumPkts = 
      new Hashtable<String,Integer>();
    ...
    for(int i=0; i<dataPkt.getSize(); i++) {
      ...
      //The source is appended with _1. Strip it
      // String tmpSource = source.substring(0,source.length()-2);
      SensorMetaData smd = 
        SensorMetaDataManager.getInstance().getSensorMetaDataIfPresent(source);

      if(smd == null){
        throw new IllegalArgumentException("No meta about the sensor \""+
      ...
      switch(datatype) {
        ...
        case Constants.DATATYPE_DOUBLE:
          ...
          for(int k=0;k<dataArr.length; k++) {
            ...
            // Its better to tag each data value with the timestamp
            // at the sensor. Now we are tagging with the timestamp
            // at the Data turbine. If delay is associated with trasfer
            // of data from the sensor to the turbine, this this result
            // may be inconsistent.
            // Disadvantages:
            // If a sensor has 'n' channels, then we have to transmit
            // 2n double values instead of just n+1 (values + time). However,
            // if we just send 'n+1', then the channels + time are coupled,
            // If we miss one 'time' value, then all channel data are useless
            ...
            Debugger.debug(Debugger.TRACE, "Adding "+dataArr[k] + " to " + 
                channel + " ("+ chData+")");
          }
          break;

        default:
          throw new IllegalStateException("Datatype of the data " +
              "unsupported for source/channel \"" +chanName);
      }
    }
  ...
  private String getTimeStringForPostGresDb(double timeSinceEpoch) {
    ...
    String timeSql = "select timestamp 'epoch' + interval '" + 
      secStr +  " seconds'";
    ...
    // Get the connection for the first database
    Connection conn = 
  ...
  public void updateIntoDb(DataPacket dataPkt) {
    ...
    Hashtable<String,SourceData> srcHash = 
      convertDataPktToTimeStampedTree(dataPkt);
    ...
    while(enumSrcData.hasMoreElements()){
      ...
      SensorMetaData smd = 
        SensorMetaDataManager.getInstance().getSensorMetaData(source);
      ...
      while(enumTsData.hasMoreElements()) {
        ...
        // fieldNamesStr = fieldNamesStr.append(Constants.LAKE_SENSORS_DB_TIME_FIELD_NAME);
        ...
        // Now get all the data for the given timestamp
        ...
        while(enumChannel.hasMoreElements()){
          ...
          if(cd.getNumDataItems() > 1) {
            throw new IllegalStateException("ERROR: Getting more than one " +
                "data values " +
                "for the same channel \"" + channel + "\" from source \"" +
                source + "\" at turbine time " + turbineTimeStr + ".");
          }
          for(int i=0;i<cd.getNumDataItems(); i++){
            Debugger.debug(Debugger.TRACE,"\t\t\tDATA [ "+i+" ] = " +
                cd.getData(i));
          }
   ...
   **/
  private void insertIntoDb(SensorMetaData smd, StringBuffer fieldNamesStr, 
  ...
  public static void main(String args[]){
    ...
    if(args.length != 2) {
      System.err.println("Usage: java stubs.LakeSink " +
          "<sinkConfig xml file> <sink name>");
      return;
    }
