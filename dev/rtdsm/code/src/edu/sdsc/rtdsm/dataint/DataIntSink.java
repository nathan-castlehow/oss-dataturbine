...
public class DataIntSink implements SinkCallBackListener{
  ...
  public DataIntSink(String sinkConfigFile, String sinkName, 
      String dbTemplateFile, String templateName){ 
    ...
    Debugger.debug(Debugger.TRACE,"Template Name: " + 
    ...
    for(int i=0; i< reqPathNameVec.size(); i++){
      ...
      for(int j=0; j< dbTables.size(); j++){
        ...
        Debugger.debug(Debugger.TRACE,"\t\tDbTable Name: " + 
            dbTables.elementAt(j));
      }
      ...
      for(int j=0; j< mapKeys.size(); j++){
        ...
        Debugger.debug(Debugger.TRACE, "\t\tMap:" + 
            mapKeys.elementAt(j) + ":" + 
            config.getMapValue(mapKeys.elementAt(j)));
      }
    }
  ...
  public void callBack(DataPacket dataPkt) {
    ...
    for(int i=0; i<dataPkt.getSize(); i++) {
      ...
      for (int j = 0; j < data.length; j++) {
        ...
        System.out.println("Received|" + sink.getName()  + "|" + chanName +
            "|" + time + "|" + "|" + j + "|" + data[j]);
        // Now update into db
  ...
  public void updateIntoDb(String chanName, Object data, long timeSinceEpoch) {
    ...
    if(config == null){
      throw new IllegalStateException("Got the data from reqPath \"" + 
          chanName + "\" for which the sink is not subscribed");
    }
    
    Vector<String> dbNamesVec = config.getDbTableNames();
    if(dbNamesVec.size() <= 0) {
      // No database to update to
      return;
    }
    ...
    switch(config.datatype){
      ...
      default:
        throw new IllegalStateException("Only double datatype is supported. " +
            "More options will be supported soon");
    }

    // TODO: Database latency should not slow down the sink from receiving new
    // messages. Hence do it in a new thread.

    for (int i=0;i < dbNamesVec.size(); i++) {

      String dbName = dbNamesVec.elementAt(i);
      String sql = "INSERT INTO " + dbName + 
       " (" + config.getMapValue(Constants.DB_TEMPLATE_SRC_TAG) + "," +
       config.getMapValue(Constants.DB_TEMPLATE_CHANNEL_TAG) + "," +
       config.getMapValue(Constants.DB_TEMPLATE_GEN_TIME_TAG) + "," +
       config.getMapValue(Constants.DB_TEMPLATE_VALUE_TAG) + ") " +
  ...
  public static void main(String args[]){
    ...
    if(args.length != 4) {
      System.err.println("Usage: java stubs.DataIntSink " +
          "<sinkConfig xml file> <sink name> <dbTemplate xml file> <template name>");
      return;
    }
    ...
    DataIntSink dswSink = new DataIntSink(sinkConfigFile, sinkName, 
        dbConfigFile, templateName);
    dswSink.connect();
  }
}
