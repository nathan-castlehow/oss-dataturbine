...
public class LakeSinkControlChannelListener implements SinkCallBackListener, Runnable {
  ...
  Hashtable<String,SensorMetaData> sensorsAddedForSink = 
    new Hashtable<String, SensorMetaData>();
  ThreadGroup threadGroup=new ThreadGroup("ControlChannel");

  LakeSinkControlChannelListener(String sinkName, 
      TurbineSinkConfig controlSinkConfig,
      SinkCallBackListener mainListener){

    this.controlSinkConfig = controlSinkConfig;
    this.actualSinkConfig = new TurbineSinkConfig(
          sinkName);
  ...
  public synchronized void callBack(DataPacket dataPkt){
    ...
    for(int i=0; i<dataPkt.getSize(); i++) {
      ...
      for (int j = 0; j < data.length; j++) {
        ...
        Debugger.debug(Debugger.TRACE, "Received|" + controlSinkConfig.getName()  + "|" + chanName +
            "|" + time + "|" + "|" + j + "|" + data[j]);
  ...
  private synchronized void restartIfNeeded(){
    if(restartNeeded){
      ...
      Debugger.debug(Debugger.TRACE, "Creation: Callback listener="+ 
          actualSink.getCallBackListener());
      actualSinkThread = new Thread(threadGroup, this, "DataSink");
      Debugger.debug(Debugger.TRACE,
          "Creating Sink thread from control Listener: " + 
          actualSinkThread);
  ...
  public void handleControlData(String msg){
    ...
    // Right now we only get webservice string.
    // If we have to extend the control information. Do it here
    if (msg.startsWith(Constants.LAKE_CONTROL_LOOKUP_PREFIX)) {
      ...
      StringTokenizer st = new StringTokenizer(msg, 
        Constants.LAKE_CONTROL_SEPARATOR);
      String srcName=null;
      ...
      if(srcName == null || serverAddr == null || username == null || 
          password == null || webSerStr == null) {

        throw new IllegalStateException("The lookup message on control channel" +
            " is corrupted");
      }
      SensorMetaData smd = SiteMetaDataParser.parse(srcName, webSerStr);
      SensorMetaDataManager.getInstance().insertMetaData(smd);
      TurbineServer server = actualSinkConfig.getTurbineServer(serverAddr, 
          username, password);
      addSinkForSensor(smd, server);
      // you have to restart the sink
  ...
  public void addSinkForSensor(SensorMetaData smd, TurbineServer server) {
    ...
    // Add it only if it is not added previously
    if(!sensorsAddedForSink.containsKey(sourceName)) {

      boolean fbReqd = true;

      // Right now enable feedback
      if(fbReqd) {
  ...
  public void run(){
    Debugger.debug(Debugger.TRACE, "ActualSink Thread: Callback listener="+ 
        actualSink.getCallBackListener());
