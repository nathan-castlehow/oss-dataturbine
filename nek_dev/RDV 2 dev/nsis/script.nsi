...
!define APPNAME "RBNB RDV installation"
!define RDVJARFILE "RDV.jar"
!define RBNBJARFILE "rbnb.jar" 
...
Section ""
  ...
  FoundVM:
  IfFileExists "$R0\bin\javaw.exe" 0 NotFound
  IfFileExists "$R0\bin\java.exe" 0 NotFound
  ...
  File "${RDVJARFILE}"
  File "${RBNBJARFILE}"
  StrCpy $R2 '"$R0\bin\java.exe" -Xmx512M -jar "${RBNBJARFILE}" '
  ExecWait "R2"

  StrCpy $R2 '$R0 -jar "${RDVJARFILE}" $R1'
  Exec "$R2 -h localhost"
