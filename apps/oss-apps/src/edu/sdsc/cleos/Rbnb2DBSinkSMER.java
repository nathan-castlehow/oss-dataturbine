...
public class Rbnb2DBSinkSMER extends RBNBBase {
	private Sink		sink	= null;
	private double rbnbStart 	= Double.MAX_VALUE;
	private double rbnbDuration = -1.0;
	private ChannelMap	map		= null;
	private static Log 	log		= LogFactory.getLog(Rbnb2DBSinkSMER.class.getName());
	...
	/**
	 * Change this to SMER name
	 * or
	 * Make it as a parameter
	 */
	private String pattern = "SMER/CLEOS cRIO/*";
	...
	public void createDBTblColNames () {
		...
		lookupTable.put("chassis temperature", new DBTableColName ("Weather", "chassis_temp"));
	    
		lookupTable.put("Dn", new DBTableColName ("Weather", "Wind_Direction_Minimum"));
		lookupTable.put("Dm", new DBTableColName ("Weather", "Wind_Direction_Average"));
		lookupTable.put("Dx", new DBTableColName ("Weather", "Wind_Direction_Maximum"));
		lookupTable.put("Hc", new DBTableColName ("Weather", "Hc"));
		lookupTable.put("Hi", new DBTableColName ("Weather", "Hi"));
		lookupTable.put("Hp", new DBTableColName ("Weather", "Hp"));
		
		lookupTable.put("Pa", new DBTableColName("Weather", "Air_Pressure")); 
		lookupTable.put("ptb210 barometer-cal", new DBTableColName ("Weather", "ptb_barom_cal"));
	    lookupTable.put("ptb210 barometer-raw", new DBTableColName ("Weather", "ptb_barom_raw"));
	    
	    lookupTable.put("Rc", new DBTableColName ("Weather", "Rain_Accumulation"));
	    lookupTable.put("Rd", new DBTableColName ("Weather", "Rain_Duration"));
	    lookupTable.put("Ri", new DBTableColName ("Weather", "Rain_Intensity"));
	    lookupTable.put("Rp", new DBTableColName ("Weather", "Rain_Peak_Intensity"));
		lookupTable.put("Sn", new DBTableColName ("Weather", "Wind_Speed_Minimum"));
		lookupTable.put("Sm", new DBTableColName ("Weather", "Wind_Speed_Average"));
		lookupTable.put("Sx", new DBTableColName ("Weather", "Wind_Speed_Maximum"));
		lookupTable.put("Ta", new DBTableColName ("Weather", "Air_Temperature"));
		lookupTable.put("Th", new DBTableColName ("Weather", "Th"));
		lookupTable.put("Tp", new DBTableColName ("Weather", "Tp"));
		lookupTable.put("Ua", new DBTableColName ("Weather", "Relative_Humidity"));
	    lookupTable.put("Vh", new DBTableColName ("Weather", "Vh"));
	    lookupTable.put("Vr", new DBTableColName ("Weather", "Vr"));
	    lookupTable.put("Vs", new DBTableColName ("Weather", "Vs"));
...
//		lookupTable.put("15", new DBTableColName ("Fake_Daq", "Ch15"));
	...
	public ChannelMap refreshChannelMap() throws SAPIException {
		...
		System.out.println ("refreshChannelMap");
		
		for(int i=0; i<this.map.NumberOfChannels(); i++) {
			System.out.println(this.map.GetName(i));
	...
	class TableForChannel {
		...
		public String generateQuery (Double timeIndex) {
			...
			System.out.println ("time is " + Long.toString((long) (timeIndex * 1000.0)));
			
			String columnValues =  "'" + SSDBOps.formatDate( (long)(timeIndex*1000.0) ) +"', ";
			...
			if (colNames.size() == 0) {
				...
				String theQuery = insert + this.tableName + "(" + columnNames
						+ ") VALUES ( " + columnValues + ")";
	...
	public ChannelMap getCleanChannelMap() throws SAPIException {
		...
		System.out.println ("HI IM IN getCleanChannelMap");
		...
		for(int i=0; i < this.map.NumberOfChannels(); i++) {
			...
			if (0 < this.map.GetTimeDuration(i) &&
				...
				System.out.println("We are about to get the duration time");
				
				channelEndTime = this.map.GetTimeStart(i) + this.map.GetTimeDuration(i);
				
				System.out.println("Here is the start time" + this.map.GetTimeStart(i));
				System.out.println("Here is the duration time" + this.map.GetTimeDuration(i));
	...
	public static void main(String[] args) {
		...
		Rbnb2DBSinkSMER thisSink = new Rbnb2DBSinkSMER();
		...
		if (thisSink.parseArgs(args) && thisSink.open()) {
			...
			try {
				...
				if (thisSink.isMonitoring()) {
					...
					thisSink.getSink().Subscribe(reqMap);
					//thisSink.getSink().Subscribe(reqMap, thisSink.lastTime, duration, "newest");
			...
			do {
				...
				try {
					...
					thisSink.getSink().Fetch(60000, reqMap);
...
/*			try {
	...
	protected String getCVSVersionString ()
	{
		return (
				...
				"$LastChangedBy:(local) $\n" +
				"$HeadURL:http://nladr-cvs.sdsc.edu/svn-private/NEON/telepresence/dataturbine/trunk/src/edu/sdsc/cleos/Rbnb2DBSinkSMER.java $\n"
		);
	}
