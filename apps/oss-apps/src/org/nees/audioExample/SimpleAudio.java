...
/**
 * This code reads a file to an audio stream, captures a sample to an
 * audio stream, plays an audio stream, and writes an audio stream to 
 * a file. These are the basic actions that one can carry out with an
 * audio stream, hence this code can serve as a tutorial example for 
 * how to do these basic audio tasks.
 * 
 * This code was stolen, copied, and otherwise cobbled together from
 * the demo sound code in Sun's sound example, JavaSoundDemo. See:
 *   http://java.sun.com/products/java-media/sound/ and
 *   http://java.sun.com/products/java-media/sound/samples/JavaSoundDemo/
 * 
 * If a file name is given as the command line argument, then that file 
 * is assumed to be a *.wav file and it is opened, read, and the sound
 * is played. If no command line argument is given. A 20 second sound
 * sample is captured, played, and writen out to Capture.wav as a 
 * *.wav file.
 * 
...
public class SimpleAudio {
    ...
    public void go(String filename) {
        ...
        if (fileIn == null) writeFileOut(captureInputStream);
    }
    ...
    /**
     * Capture 20 seconds of live audio to a ByteArrayOutputStream
     * in any failure case print a message and return null
     */
    private AudioInputStream captureLiveAudio()
    {
        ...
        AudioInputStream audioInputStream = 
            new AudioInputStream(bais, selectedFormat, audioBytes.length / frameSizeInBytes);

        long milliseconds = 
            (long)((audioInputStream.getFrameLength() * 1000) / selectedFormat.getFrameRate());
    ...
    private void playback(AudioInputStream streamToPlay) {
        ...
        AudioInputStream playbackInputStream = 
            AudioSystem.getAudioInputStream(selectedFormat, streamToPlay);

        if (playbackInputStream == null) {
            System.out.println("Unable to convert stream of format " + streamToPlay + " to format " + selectedFormat);
            return;
        }
    ...
    private void writeFileOut(AudioInputStream streamToWrite) {
        // ----------------------------------------------------------------------
        // Write results out to a file (specified by the static varible OUT_FILE)
