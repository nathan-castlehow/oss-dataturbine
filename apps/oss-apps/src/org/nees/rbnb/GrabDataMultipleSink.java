...
public class GrabDataMultipleSink extends RBNBBase {
	...
	protected Options setOptions() {
		...
		setNotes("Writes data between start time and end time to " +
			"the a directory structure on the given directory " +			"such that each directory reflects the channel path of origin. " +
	...
	{	
		...
		private void initializeFile() throws IOException
		{
			String path = archiveDirectory + 
				File.separator + fullPath.replace('/',File.separatorChar);
			File f = new File(path);

			// make sure that the directories on the path exist
		...
		private void runWork () 
		{
			try
			{
				while(isRunning())
				{
					System.out.println("Before fetch " + sink.GetServerName() +
						" " + sink.GetClientName());
					...
					System.out.println("fetch for " + fullPath + 
						" " + times.length + " " + data.length);
					for (int i = 0; i < times.length; i++)
						writeLineToFile(times[i], data[i]);
				}
			}
			catch (Throwable t)
			{
