...
//    computeDefaultTimeOffset();
  ...
  //! Util compute the default time offset taking into account
...
/*        
    System.out.println("setArg() - Starting FileToRbnb on " + getServer()
      + " as " + sourceName);
*/    
          ...
          int rbnbArchiveSize)
  {
    ...
    if ((archiveSize > 0) && (archiveSize < cacheSize)){
      System.out.println("Archive size = " + archiveSize + "; it must be " +
            "bigger than cache size; cache size = " + cacheSize);
        return false;
    }
/*    
    System.out.println("setArgs - Starting FileToRbnb on " + getServer()
        + " as " + sourceName);
*/    
    System.out.println("  Channel name = " + channelName
        + "  Cache Size = " + cacheSize + "; Archive size = " + archiveSize);
  ...
  public void connect()
  {
    ...
    try {
      ...
      connected = true;
      System.out.println("FileToRbnb - connect() : Connection made to server = "
        + getServer() + " as " + sourceName + " with " + channelName + ".");
  ...
  public void doIt () {
    ...
    try {
      ...
      while (((in = rd.readLine()) != null) && connected) {
        ...
        // the first valid token is the time stamp
        while (st[index].length() == 0)
          index++;
        ...
        if (!timestamp.is_valid) {
          if (log.isWarnEnabled()) {
            log.warn("Warning: timestamp not valid: "
                + tsString);
          }
        }
        
        double time = timestamp.getValue();  // in seconds
...
//          System.out.println(channelNameArray[j] +" got value: " + data[0]);
          // post data to channel i
          try{
              chanIndex = map.GetIndex(channelNameArray[j]);
//              System.out.println("Channel Index: " + chanIndex);
      ...
      //==== Following used to be old way of posting user data
//      StringBuffer unitsStr = new StringBuffer();
//      getUnitsString(channelNameArray, unitsStr);
//      putUnitsString(unitsStr.toString());
      //======  Not need to do above === //

      // post units using PutUserInfo() 
      String unitsArray[];
...
//      System.out.println(name+":\t"+value);
  ...
  public int getChannelCount() {
  	if (headers.containsKey("CHAN_COUNT"))
  		return (Integer.parseInt(headers.getProperty("CHAN_COUNT")));
  ...
  void populateChannelMap(ChannelMap map, String channelNames[]) throws SAPIException {
    ...
    for (int i = 0; i < st.length; i++) {
      ...
      if (token.compareToIgnoreCase("time") != 0 &&
        token.compareToIgnoreCase("time_ms") != 0 &&
        token.length() > 0) {
...
//        System.out.println("postUnits - units=" + units[i] + " for index " + index[i]);
      } catch (SAPIException se) {
