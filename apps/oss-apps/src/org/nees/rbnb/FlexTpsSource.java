...
public class FlexTpsSource extends MJPEGSource {
  ...
  private String completeCameraURLwarningMsg = "ERROR: the \'R\' parameter is mutually exclusive the \'A\', " +
  "\'F\', \'N\',and \'T\' parameters.";
  ...
  private final static String DEFAULT_VERSION = "1.x.x";
  private String urlVersion = DEFAULT_VERSION;
	...
	protected Options setOptions() {
		...
		opt.addOption("F", true, "Frames per second *" + DEFAULT_FPS
				+ " (mutually exclusive with R)");
		
		opt.addOption("N", true,
						"User defined stream name (required - mutually exclusive with R)");
		
		opt.addOption("T", true,
				"Feed name (required - mutually exclusive with R)");

		opt.addOption("R", true,
						"flexTPS video source URL (required - mutually exclusive with A, F, N, and T)");
	...
	protected boolean setArgs(CommandLine cmd) {
		...
		if (cmd.hasOption('F')) {
			...
			if (a != null)
				try {
					...
					System.out
							.println("Please enter a numeric value for frames per second value (-F option). "
									+ a + " is not valid!");
					return false;
				}
		}
		...
		if (cmd.hasOption('R')) {
			...
			if (cmd.hasOption('A') || cmd.hasOption('F') || cmd.hasOption('N')
					|| cmd.hasOption('T')) {
				System.err.println(completeCameraURLwarningMsg);
				return false;
			}
			...
			if (a != null) {
				...
				cameraFPS = Integer
						.parseInt(urlSplit[urlSplit.length - 1]);

				System.out
						.println("WARNING: use of the \'R\' option will not allow robotic PTZ camera control by rbnb clients.");
			}
		...
		if (getHostName() == null && !this.hasCompleteCameraURL) {
			System.err
					.println("TPS camera hostname or complete URL is required. Use FlexTpsSource -h for help");
			printUsage();
			return false;
		}
		...
		if (urlType == null && !this.hasCompleteCameraURL) {
			System.err
					.println("TPS camera stream type is required. Use FlexTpsSource -h for help");
			printUsage();
			return false;
		}
		...
		if (urlStreamName == null && !this.hasCompleteCameraURL) {
			System.err
					.println("TPS camera stream name is required. Use FlexTpsSource -h for help");
			printUsage();
			return false;
		}

		if (!this.hasCompleteCameraURL) { // then build one from the bits
			if (urlVersion.equals("0.4.x")) {
	       cameraURLString = "http://" + getHostName() + "/feeds/"
	      + urlType + "/nph-mjpeg_stream.pl?stream="+ urlStreamName;
	       
			} else {
        cameraURLString = "http://"
          + getHostName()
          + "/feeds/"
          + urlType
          + "/" 
		...
		System.out.println("Starting FlexTpsSource with... \n"
				+ " flexTPS video source URL = "
				+ ((this.hasCompleteCameraURL) ? mjpegURL : cameraURLString)
  ...
  protected URLConnection getCameraConnection() {
	...
	private static double parseISO8601Date(String dateString) {
		try {
			...
			String subSecondsString = dateString.substring(20, dateString
					.length() - 1);
