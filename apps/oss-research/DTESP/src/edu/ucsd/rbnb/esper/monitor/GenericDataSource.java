...
public class GenericDataSource {
	...
	private final Source source;					//Source to write to
	private ChannelMap cmap;						//Channel map to store ch in
	private HashMap<String, Integer> chRecorder;	//Stores ch names
	...
	public GenericDataSource(String srcName) throws SAPIException{
		this(srcName, 	DEFAULT_RBNB_SERVER, DEFAULT_RBNB_PORT, 
						DEFAULT_CACHESIZE, DEFAULT_ARCHIVESIZE);
	 ...
	 */
	public GenericDataSource( String srcName, String serverPath, int port,
				int cacheSize, int archiveSize)	 
				throws SAPIException {
		
		this(srcName, 	serverPath, port, cacheSize, archiveSize,
						DEFAULT_ARCHIVEMODE, true);
	 ...
	 */
	public GenericDataSource(	String srcName, String serverPath, int port,
				int cacheSize, int archiveSize, boolean autoOpenConnection)	 
				throws SAPIException {
		
		this(srcName, 	serverPath, port, cacheSize, archiveSize,
						DEFAULT_ARCHIVEMODE, autoOpenConnection);
	 ...
	 */
	public GenericDataSource(	String srcName, String serverPath, int port,
			int cacheSize, int archiveSize, String archiveMode, boolean autoOpenConnection)	 throws SAPIException {
	...
	public synchronized void close(){
		try {flush(true);} catch (Exception e) {}
	...
	private int getCh(String name){
		...
		if(i == null)
			throw new IllegalStateException("Channel '"+name+"' not set");
